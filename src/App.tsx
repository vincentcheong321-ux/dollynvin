      ...
      <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl z-10 p-6 sm:p-8 animate-fadeIn flex flex-col max-h-[90vh] border border-slate-100">
        ...
        <div className="space-y-6 overflow-y-auto pr-2 flex-1 no-scrollbar">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
             ...
             <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Cost</label>
                <input type="number" placeholder="0" value={formData.cost === undefined ? '' : formData.cost} onChange={e => setFormData({...formData, cost: parseFloat(e.target.value) || 0})} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-medium" />
             </div>
          </div>
        </div>
...
// --- Budget Modal ---
const BudgetModal = ({ isOpen, onClose, trip }: { isOpen: boolean, onClose: () => void, trip: Trip }) => {
  if (!isOpen) return null;
  let total = 0;
       ...
       <div className="bg-white w-full max-w-xl rounded-3xl shadow-2xl z-10 p-8 flex flex-col max-h-[85vh] animate-slideUp">
         ...
         <div className="overflow-y-auto pr-2 space-y-8 flex-1 no-scrollbar">
            <div className="bg-rose-600 rounded-2xl p-8 text-white text-center shadow-lg">
               <p className="text-white/80 font-bold uppercase tracking-widest text-xs mb-2">Total Estimated Cost</p>
               <h2 className="text-5xl font-serif font-bold">{total.toLocaleString()}</h2>
            </div>
...
const App = () => {
  ...
  const addDay = () => {
    ...
    const newDay: DailyPlan = { id: generateId(), dayNumber: newDayNum, activities: [], theme: `Day ${newDayNum}` };
  ...
  return (
    <div className="min-h-screen flex flex-col">
       <header className="bg-white/90 backdrop-blur-md sticky top-0 z-50 border-b border-rose-100">
         <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center space-x-1">
               <button onClick={() => setIsBudgetOpen(true)} className="p-2 text-rose-400"><WalletIcon className="w-5 h-5" /></button>
               <button onClick={() => setIsNotesOpen(!isNotesOpen)} className={`p-2 rounded-full ${isNotesOpen ? 'bg-rose-100 text-rose-600' : 'text-rose-400'}`}><NoteIcon className="w-5 h-5" /></button>
            </div>
         </div>
         <div className="w-full bg-white/50 py-3 px-4 flex flex-wrap justify-center gap-2 border-t border-rose-50 no-scrollbar overflow-x-auto">
            {trip.dailyPlans.map(p => (
              <button key={p.id} onClick={() => { setActiveDay(p.dayNumber); setIsNotesOpen(false); }} className={`min-w-[3rem] p-2 rounded-xl text-xs font-bold border transition-all ${activeDay === p.dayNumber && !isNotesOpen ? 'bg-rose-600 border-rose-600 text-white' : 'bg-white border-rose-100 text-rose-300'}`}>
                Day {p.dayNumber}
              </button>
            ))}
       ...
       <main className="flex-1 max-w-3xl mx-auto w-full p-4 pb-32">
         {isNotesOpen ? (
           <div className="bg-white rounded-3xl p-8 shadow-sm border border-rose-50 space-y-6 animate-fadeIn">
           ...
           <div className="space-y-6 animate-fadeIn">
             <div className="flex items-center justify-between p-2">
                ...
                <div className="text-right">
                   <div className="text-[10px] font-bold text-rose-400 uppercase tracking-widest mb-1">Day Budget</div>
                   <div className="font-bold text-rose-900 text-xl">{dayTotal.toLocaleString()}</div>
                </div>
             </div>
             ...
             <div className="space-y-4">
               ...
               {sortedActivities.map(act => (
                 <div key={act.id} onClick={() => { setEditingActivity(act); setIsActivityModalOpen(true); }} className={`group bg-white p-5 rounded-3xl shadow-sm border transition-all cursor-pointer hover:shadow-md ${act.isBooked ? 'border-l-4 border-l-green-400' : 'border-rose-50'}`}>
       ...
       <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-white via-white/80 to-transparent pointer-events-none">
          <div className="max-w-3xl mx-auto flex justify-center pointer-events-auto">
             <button onClick={() => { setEditingActivity(null); setIsActivityModalOpen(true); }} className="bg-rose-600 text-white px-8 py-4 rounded-2xl font-bold shadow-xl hover:bg-rose-700 hover:scale-105 transition-all flex items-center gap-2">
                <PlusIcon className="w-5 h-5" /> Add to Journey
             </button>
          </div>
       </div>
       ...
       <BudgetModal isOpen={isBudgetOpen} onClose={() => setIsBudgetOpen(false)} trip={trip} />
    </div>
  );
};
